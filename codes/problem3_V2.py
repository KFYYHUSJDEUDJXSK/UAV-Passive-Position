...
for iter in range(iter_max):
    # 随机选择两个无人机
    launch_1=random.randint(1, 9)
    # launch_1 += 1
    # launch_1 = launch_1 % 9 + 1
    while 1:
        launch_2 = random.randint(1, 9)
        if launch_2 != launch_1:
            break

    # 发射信号,其余无人机得到角度
    for i in range(1,10):
        if (i == launch_1) or (i == launch_1):
            continue

        # 角度0-i-launch_1
        alpha_1 = get_angle(np.array([plane[0,:],plane[i,:],plane[launch_1,:]]))
        # 角度0-i-launch_2
        alpha_2 = get_angle(np.array([plane[0, :], plane[i, :], plane[launch_2, :]]))
        ...
        angle[1] = alpha_2
        # beta
        angle[2] = (launch_1 - 1) * 2 / 9 * np.pi
        angle[3] = (launch_2 - 1) * 2 / 9 * np.pi
